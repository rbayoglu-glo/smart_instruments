...
def main() -> None:
    ...
    for i, ((file_name, intervention), ax) in enumerate(zip(cases, axes)):
        ...
        if idx.size > 0:
            h_start = ax.scatter(
                ...
                linewidth=1.1,
                zorder=4,
                label="ramp start (on filtered force, preload included)",
            )
        ...
        ax.set_title(
            f"{file_name} | {intervention}   filtered sync check + ramp start "
    ...
    fig.suptitle(
        ...
        "theta0 calibration excludes low-force blocks that look like genuine non-neutral holds "
        "(|block median| > "
        f"{THETA0_BLOCK_EXCLUDE_THETA_DEG:g} deg); theta shown is theta_sm - theta0.",
        fontsize=12,
    )
    fig.tight_layout()
    out = OUT_DIR / "sync_check_all_interventions_stacked_filtered_rampstart_theta0corr.png"
