...
def load_synced_case(file_name: str) -> dict:
    ...
    disp = ls.theta_deg_to_tip_mm(th)
        ...
        cycles.append(
            {
                ...
                "disp_start_mm": float(disp[i0]),
                "disp_peak_mm": float(disp[pk]),
                "ddisp_mm": float(disp[pk] - disp[i0]),
...
def main() -> None:
    ...
    if SHOW_IDENTIFICATION_OVERLAYS:
        ...
        cyc["transition_flag_raw"] = False
        cyc["transition_flag"] = False
        print("\nTransition/start identification overlays are disabled for this experimental run.")

    out_csv = OUT_DIR / "sync_check_transition_flags_raw_force_displacement.csv"
    ...
    for i, ((file_name, intervention, case), ax) in enumerate(zip(case_data, axes)):
        ...
        ax2 = ax.twinx()
        h_theta, = ax2.plot(t, disp, lw=0.9, color="tab:orange", label="tip displacement")
        ax2.set_ylabel("tip displacement [mm]", color="tab:orange")
    ...
    fig.suptitle(
        (
            ...
            "raw force/displacement/time only (identification overlays disabled)"
        ),
        fontsize=12,
    )
    fig.tight_layout()
    out = OUT_DIR / "sync_check_all_interventions_stacked_raw_force_displacement.png"
