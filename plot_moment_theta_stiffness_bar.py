...
def main() -> None:
    ...
    out_dir = repo_root / "output" / "moment_theta_cycle_grids"
    summary_csv = out_dir / "all_interventions_moment_theta_stiffness_summary.csv"
    ...
    x = range(len(df))
    y = df["k_mean_nm_per_deg"].to_numpy()
    yerr = df["k_std_nm_per_deg"].to_numpy()
    ...
    ax.set_title("Moment-Theta Stiffness by Intervention (Mean +/- SD)")
    ...
    fig.tight_layout()
    out_png = out_dir / "all_interventions_moment_theta_stiffness_mean_std_bar_by_order.png"
