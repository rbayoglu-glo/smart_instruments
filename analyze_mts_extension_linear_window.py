...
def extract_centered_positive_extension_branch(
...
def analyze_intervention(intervention: str, book_num: int) -> dict:
    ...
    theta_branch, moment_branch, theta_avg, moment_avg, branch_reason = extract_centered_positive_extension_branch(
        angle_cycle,
        moment_cycle,
    )
    if branch_reason != "ok":
...
def plot_bar(df: pd.DataFrame, out_png: Path) -> None:
    ...
    ax.set_title(f"MTS only (Extension): two-point secant stiffness ({int(M_LOW_NM)} to {int(M_HIGH_NM)} Nm)")
...
def plot_method_comparison_bar(df: pd.DataFrame, out_png: Path) -> None:
    ...
    ax.set_ylabel("Stiffness [Nm/deg]")
    ax.set_title("MTS Extension: stiffness comparison grouped by method")
    ax.grid(axis="y", linestyle="--", alpha=0.3)
...
def main() -> None:
    ...
    out_fit = OUT_DIR / f"mts_extension_{WINDOW_TAG}_secant_fit_panels.png"
    ...
    out_csv = OUT_DIR / f"mts_extension_{WINDOW_TAG}_secant_stiffness.csv"
    df.to_csv(out_csv, index=False)
    print(f"saved {out_csv}")

    out_bar = OUT_DIR / f"mts_extension_{WINDOW_TAG}_secant_stiffness_bar.png"
    plot_bar(df, out_bar)

    out_compare_bar = OUT_DIR / "mts_extension_stiffness_four_methods_bar.png"
    ...
    if not valid.empty:
        ...
        print(f"\nMTS extension stiffness ranking ({int(M_LOW_NM)}-{int(M_HIGH_NM)} Nm two-point secant):")
